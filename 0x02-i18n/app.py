...
def get_timezone() -> str:
    """A function that infers users timezone time zone."""
    timezone_from_url = request.args.get('timezone', None)
    if timezone_from_url is not None:
        try:
            timezone_obj = pytz.timezone(timezone_from_url)
            return timezone_from_url
        except Exception as e:
            pass
    if g.user:
        timezone_from_user = g.user.get('timezone', None)
        if timezone_from_user is not None:
            try:
                timezone_obj = pytz.timezone(timezone_from_user)
                return timezone_from_user
            except Exception as e:
                pass
...
def home():
    ...
    current_time = now(pytz.timezone(timezone)).strftime('%Y-%m-%d %H:%M:%S')
    return render_template('index.html', g=g, time=current_time)
